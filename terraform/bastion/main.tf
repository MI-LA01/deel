...
// Allow access to the Bastion Host via SSH.
resource "google_compute_firewall" "bastion-ipsec" {
  name          = format("%s-bastion-ipsec", var.bastion_name)
  network       = var.network_name
  direction     = "INGRESS"
  project       = var.project_id
  source_ranges = ["0.0.0.0/0"] // TODO: Restrict further.
...
data "template_file" "startup_script" {
  ...
  sudo apt-get install -y tinyproxy wget
  wget https://get.vpnsetup.net -O /tmp/vpn.sh
  sudo sh /tmp/vpn.sh
  EOF
}
...
resource "google_compute_instance" "bastion" {
  ...
  // Install tinyproxy and ipsec vpn on startup.
